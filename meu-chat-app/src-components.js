...
export const TelaChat = ({ contato, msgs, onVoltar, onEnviar, onFotoCamera, onFotoGaleria, onZoom, onVerPerfil, onDenunciar, contatoOnline }) => {
  ...
  return (
    <View style={{flex:1, backgroundColor:'#EFEFF4'}}>
      ...
      <FlatList
        ref={flatListRef} data={msgs} keyExtractor={item => item.id}
             ...
             <View style={[styles.msgRow, isEu ? {justifyContent:'flex-end'} : {justifyContent:'flex-start'}]}>
               <View style={[styles.bubble, isEu ? styles.bubbleEu : styles.bubbleEles]}>
                 ...
                 <View style={styles.msgMeta}>
                     <Text style={[styles.msgTime, isEu ? {color:'rgba(255,255,255,0.7)'} : {color:'#999'}]}>{new Date(parseInt(item.id)).toLocaleTimeString().slice(0,5)}</Text>
...
export const AlertaGlobal = ({ texto, onFechar }) => {
    if(!texto) return null;
    return (
        <View style={styles.alertaContainer}>
            <View style={styles.alertaBox}>
                <Ionicons name="megaphone" size={24} color="white" />
                <Text style={styles.alertaTexto}>{texto}</Text>
                <TouchableOpacity onPress={onFechar}><Ionicons name="close" size={20} color="white"/></TouchableOpacity>
            </View>
        </View>
    )
}